...
const comentar = async (req, res) => {    
    ...
    if(!texto){
        return res.status(404).json({mensagem: 'Para comentar nessa postagem é necessario  informar o texto'});
    }
    ...
    try {
        ...
        const comentario = await knex('postagem_comentarios').insert({usuario_id: id, postagem_id: postagemId, texto: texto}).returning('*');
